...
public class AttendeeService {
    private final AttendeeRepository attendeeRepository; // O repositório para participantes
    private final CheckInService checkInService;

    // Obtém todos os participantes de um evento com base no ID do evento
    public List<Attendee> getAllAttendeesFromEvent(String eventId) {
        return this.attendeeRepository.findByEventId(eventId);
    }


    // Obtém detalhes dos participantes de um evento
        ...
        List<AttendeeDetails> attendeeDetailsList =
           ...
           Optional<CheckIn> checkIn = this.checkInService.getCheckIn(attendee.getId());
            LocalDateTime checkedInAt = checkIn
                                        .<LocalDateTime>map(CheckIn::getCreatedAt)
                                        .orElse(null);
